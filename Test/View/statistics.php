<?php
// Funcția pentru a genera conținutul CSV și a-l descărca
function exportToCSV($data, $filename) {
    // Setăm antetul pentru descărcare
    header('Content-Type: text/csv');
    header('Content-Disposition: attachment; filename="' . $filename . '"');

    // Deschidem pointerul către output-ul standard (cel mai frecvent outputul browser-ului)
    $output = fopen('php://output', 'w');

    // Scriem rândurile din date în fișierul CSV
    foreach ($data as $row) {
        fputcsv($output, $row);
    }
...
// Verificăm dacă utilizatorul a făcut clic pe butonul de export CSV
if (isset($_POST['export_csv'])) {
    ...
    switch ($option) {
        ...
        case 'option4':
            ...
            break;
        default:
            // Tratează cazul în care nu există o opțiune validă
            break;
    }
}
?>
